...
1. **HTML5语义化标签** → [01-语义化标签.html](../HTML教学演示/01-语义化标签.html)
   - 什么是语义化标签：语义化标签指的是在HTML5里面用合适的标签来描述内容的含义，而不仅仅是外观
   - 常用语义化标签：header、footer、nav、main、section、article、aside
   - 语义化的优点：有利于SEO的优化，有利于提高团队协作开发，便于屏幕阅读器的解析，有利于提高代码的可读性
...
3. **HTML5表单** → [05-表单验证.html](../HTML教学演示/05-表单验证.html)
   - 常用表单元素：input（让用户输入或选择内容，输入文字，填密码，选文件）、textarea（多行文本输入框）、select(下拉选择框)、button（按钮元素）
   - HTML5新的input类型：email、number、date、tel、url
   - 表单验证？（required：必填项有没有填，格式对不对，长度合不合适，）pattern（正则表达式）、min/max
   - 表单提交和阻止默认行为(表单提交的默认方法：type=submit，type=reset方法)
...
3. **Flex弹性布局** → [CSS高频面试题汇总.md](./CSS高频面试题汇总.md#三display属性详解)
...
2. **ref vs reactive** → [Vue面试题精简版.md](./Vue面试题精简版.md#4-ref-vs-reactive-)
   ...
   - reactive：
     ...
     - 直接访问属性
     - 解构会失去响应性（需要toRefs）
   - toRefs：将响应式对象的属性转换为 ref：（ toRefs 批量转换）
